...
class DocumentProcessor:
    ...
    def check_file_exists(self, filename: str) -> bool:
        ...
        possible_paths = [
            os.path.join(config.CORE_DATA_FOLDER, filename),
            os.path.join(config.CORE_DATA_FOLDER, base_filename + ".txt"),
            os.path.join(config.CORE_DATA_FOLDER, base_filename + ".md")
        ]
...
async def upload_document_and_schedule_processing(
    ...
    try:
        ...
        os.makedirs(config.CORE_DATA_FOLDER, exist_ok=True)
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(filename)[1]) as temp_file:
            ...
            try:
                ...
                output_path = os.path.join(config.CORE_DATA_FOLDER, output_filename)
...
from rag_components import _create_weaviate_schema_if_not_exists, _ingest_chunks_with_native_batching
...
def full_process_and_ingest_pipeline(filepath: str, file_hash: str, embedding_model):
    ...
    try:
        ...
        _ingest_chunks_with_native_batching(weaviate_client, collection_name, processed_chunks, embeddings_model)
